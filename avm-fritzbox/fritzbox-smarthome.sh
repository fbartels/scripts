...
if [ -z $1 ]; then
	ainlist=$(curl -s "http://$avmfbip/webservices/homeautoswitch.lua?sid=$avmsid&switchcmd=getswitchlist")
else
	ainlist=$1
fi
...
for ain in $ainlist; do
	...
	ainstate=$(curl -s "http://$avmfbip/webservices/homeautoswitch.lua?sid=$avmsid&ain=$ain&switchcmd=getswitchstate")
	case $ainstate in
		1) ainstate2=on;;
		0) ainstate2=off;;
		*)
	...
	ainname=$(curl -s "http://$avmfbip/webservices/homeautoswitch.lua?sid=$avmsid&ain=$ain&switchcmd=getswitchname")

	if [ ! -z $2 ]; then
		# force switchcmd if explicitly given (non-interactive mode)
		case $2 in
			on) curl -s "http://$avmfbip/webservices/homeautoswitch.lua?sid=$avmsid&ain=$ain&switchcmd=setswitchon" > /dev/null;;
			off) curl -s "http://$avmfbip/webservices/homeautoswitch.lua?sid=$avmsid&ain=$ain&switchcmd=setswitchoff" > /dev/null;;
			*)
		...
		read -p "Do you want to toggle $ainname ($ain)? Switch is currently $ainstate2. (y/n) " -n 1 -r
			echo
			if [[ $REPLY =~ ^[Yy]$ ]]; then
					switchtoggle=$(curl -s "http://$avmfbip/webservices/homeautoswitch.lua?sid=$avmsid&ain=$ain&switchcmd=setswitchtoggle")
